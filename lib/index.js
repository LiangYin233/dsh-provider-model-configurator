/**
 * dsh-model-config-sync host plugin: mounts the `modelConfigSync` Typert
 ...
 * /plugins/dsh-model-config-sync/client.js and it registers the "Model
 * Config Sync" settings page.
 *
...
import { THINKING_LEVELS, TYPERT_MANIFEST } from './contract.js'

/** Cordis plugin name (the Loader entry and client bundle id). */
export const name = 'dsh-model-config-sync'
...
/**
 * The modelConfigSync Remote service. Each method answers the business
 * envelope `{ ok: true, ... }` or `{ ok: false, error }`; the typert
 * boundary wraps it in its own transport envelope.
 */
class ModelConfigSyncRuntime extends TypertRemoteService {
  /**
   * Register the service under the `modelConfigSync` key (the wire namespace).
   * @param ctx - owning cordis context.
   */
  constructor(ctx) {
    super(ctx, 'modelConfigSync')
  }
  ...
  async applyModelConfig(route, entry, overwrite) {
    const st = this.ctx.get('settings')
    if (st === undefined) return { ok: false, error: 'settings 服务不可用' }
    ...
    if (idx >= 0) {
      if (overwrite !== true) return { ok: false, error: `模型 "${id}" 已存在;请勾选「覆盖现有同名模型」后重试` }
...
/**
 * Mount the modelConfigSync service and its strict Typert manifest.
 * @param ctx - host cordis context.
 */
export function apply(ctx) {
  new ModelConfigSyncRuntime(ctx)
  ctx.effect(() => ctx.typert.register(TYPERT_MANIFEST), 'dsh-model-config-sync: typert manifest')
}
