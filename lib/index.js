...
class ModelConfiguratorRuntime extends TypertRemoteService {
  ...
  async presetModels(provider) {
    ...
    let live = new Set()
    try { live = new Set(llm.listProviders().map((p) => p.id)) } catch { /* provider list unavailable; treat all as dormant */ }
    try {
      let models
  ...
  async presetModelInfo(provider, model) {
    ...
    try {
      ...
      if (hit) {
        ...
        // Keep the catalog path in sync with presetModels: input modalities are
        // advertised by the catalog too, not only by the resolve path.
  ...
  async applyModelConfig(route, entry, overwrite, clearFields) {
    ...
    const profile = providers[route]
    // hasOwnProperty guards prototype keys (e.g. "__proto__") from ever being
    // treated as a configured provider route.
    ...
    // data); normalize them so the spread below never turns a string into an
    // index-keyed object (which would corrupt the whole models list on write).
