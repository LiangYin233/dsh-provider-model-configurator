/**
 * dsh-model-config-sync client bundle: the "Model Config Sync" settings page.
 *
 ...
 * file raw under /plugins/dsh-model-config-sync/client.js. The ModuleLoader
 ...
 * writes explicit model entries through the host `modelConfigSync` Typert
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var NS = 'settings.model-config-sync'
    ...
    function codec(name, sch) { return { mode: 'strict', typeSymbol: 'dsh-model-config-sync#' + name, schema: sch } }
    ...
    var STYLE_ID = 'dsh-model-config-sync-styles'
      ...
      return el('div', { className: 'mcfg-page' },
        ...
        el('section', { className: 'mcfg-card' },
          ...
          el('div', { className: 'mcfg-field' },
            ...
            el('select', { className: 'mcfg-input mcfg-selectInput', value: targetRoute, onChange: function (e) { setTargetRoute(e.target.value) } },
          ...
          target ? el('div', { className: 'mcfg-field' },
            ...
            el('input', {
              ...
              onChange: function (e) { set({ id: e.target.value }); setOverwrite(false) },
            }),
        ...
        el('section', { className: 'mcfg-card' },
          ...
          el('div', { className: 'mcfg-row' },
            el('div', { className: 'mcfg-field', style: { flex: '1' } },
              ...
              el('input', { className: 'mcfg-input', value: form.id, onChange: function (e) { set({ id: e.target.value }); setOverwrite(false) } }),
            ),
            el('div', { className: 'mcfg-field', style: { flex: '1' } },
    ...
    // ---- plugin ----
    exports.name = 'dsh-model-config-sync'
    exports.inject = ['slots', 'remote', 'locale']
    exports.apply = function (ctx) {
      ctx.effect(function () { return ctx.locale.register(NS, { zh: zh, en: en }) }, 'dsh-model-config-sync: dictionaries')
      adoptStyles()
      var t = ctx.locale.bind(NS)

      // Mount the modelConfigSync Remote namespace, then resolve its handle
      ...
      ctx.effect(async function () {
        var dispose = await ctx.remote.$mount({ package: 'dsh-model-config-sync', descriptors: INVOCATIONS })
        var handle = ctx.reflect.get('remote.modelConfigSync')
        if (handle === undefined) {
          throw new Error('dsh-model-config-sync: the modelConfigSync Remote namespace did not mount')
        }
        remote = handle
        return function () { remote = null; void dispose() }
      }, 'dsh-model-config-sync: remote')
      ...
      ctx.slots.inject('settings.section', function () {
        return ctx.slots.register(
          { name: 'settings.section', id: 'model-config-sync', order: 11, label: function () { return t('nav') } },
          function () { return React.createElement(ModelConfigSyncPage, { t: t, call: call }) },
        )
      })
    }
