...
var en = {
  ...
  customTag: "custom",
  targetTitle: "2. Target provider \xB7 model management",
...
var css = ".mcfg-page{display:flex;flex-direction:column;gap:16px;max-width:680px;padding:4px 2px 24px}.mcfg-title{color:var(--dsw-alias-label-primary);font-size:16px;font-weight:500;line-height:24px;margin:0}.mcfg-intro{color:var(--dsw-alias-label-secondary);font-size:13px;line-height:20px;margin:0}.mcfg-card{background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l1);border-radius:12px;padding:14px 16px;display:flex;flex-direction:column;gap:12px}.mcfg-cardTitle{color:var(--dsw-alias-label-primary);font-size:14px;font-weight:500;line-height:20px;margin:0}.mcfg-field{display:flex;flex-direction:column;gap:6px}.mcfg-label{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:16px}.mcfg-input{box-sizing:border-box;width:100%;height:32px;padding:0 10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-family:inherit;font-size:13px}.mcfg-input:focus{outline:none;border-color:var(--dsw-alias-brand-primary)}.mcfg-input:disabled{opacity:.55}.mcfg-selectInput{appearance:auto}.mcfg-shrink{width:auto;flex:none}.mcfg-row{display:flex;flex-direction:row;gap:8px;align-items:center}.mcfg-hint{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;margin:0}.mcfg-note{color:var(--dsw-alias-state-warn-primary);font-size:12px;line-height:18px;margin:0}.mcfg-divider{border-top:1px solid var(--dsw-alias-border-l1);padding-top:12px;margin-top:4px}.mcfg-info{display:flex;flex-direction:column;gap:4px;border-left:2px solid var(--dsw-alias-border-l1);padding-left:10px}.mcfg-infoLine{color:var(--dsw-alias-label-secondary);font-size:12px;line-height:18px;margin:0}.mcfg-btn{box-sizing:border-box;height:32px;padding:0 14px;border-radius:8px;border:1px solid var(--dsw-alias-border-l1);background:var(--dsw-alias-bg-base);color:var(--dsw-alias-label-primary);font-family:inherit;font-size:13px;cursor:pointer}.mcfg-btn:hover{background:var(--dsw-alias-interactive-bg-hover,var(--dsw-alias-bg-layer-1))}.mcfg-btnPrimary{box-sizing:border-box;height:32px;padding:0 16px;border-radius:8px;border:none;background:var(--dsw-alias-brand-primary);color:#fff;font-family:inherit;font-size:13px;font-weight:500;cursor:pointer}.mcfg-btnPrimary:disabled{opacity:.5;cursor:default}.mcfg-check{display:flex;flex-direction:row;gap:8px;align-items:center;color:var(--dsw-alias-label-primary);font-size:13px;cursor:pointer}.mcfg-statusOk{color:var(--dsw-alias-state-success-primary);font-size:13px;line-height:20px;margin:0}.mcfg-statusErr{color:var(--dsw-alias-state-error-primary);font-size:13px;line-height:20px;margin:0}.mcfg-code{background:var(--dsw-alias-bg-base);border:1px solid var(--dsw-alias-border-l1);border-radius:8px;padding:10px;font-family:ui-monospace,Consolas,monospace;font-size:12px;line-height:18px;white-space:pre-wrap;word-break:break-all;color:var(--dsw-alias-label-secondary);max-height:260px;overflow:auto;margin:0}";
...
function ModelConfiguratorPage(props) {
  ...
  React.useEffect(() => {
    let alive = true;
    Promise.all([call("preset-providers"), call("target-providers")]).then((results) => {
      if (!alive) return;
      const a = results[0] || {};
      const b = results[1] || {};
      const errs = [];
      ...
      if (alive) setBoot((x) => ({ ...x, error: String(err && err.message || err) }));
    });
    return () => {
      alive = false;
    };
  ...
  const removeModel = (modelId) => {
    ...
    setDeleting(true);
    setStatus(null);
    call("delete-model", { route: targetRoute, modelId }).then((r) => {
      if (!r || r.ok !== true) {
      ...
      call("target-providers").then((b) => {
        if (b && b.ok === true) setBoot((x) => ({ ...x, targets: b.providers }));
      }).catch(() => {
      });
    }).catch((err) => setStatus({ kind: "err", text: String(err && err.message || err) })).finally(() => setDeleting(false));
  };
  const clearSource = () => {
    setSourceProvider("");
    setSourceModel("");
  ...
  const onSourceProvider = (value) => {
    ...
    if (!value) return;
    setBusyModel(true);
    call("preset-models", { provider: value }).then((r) => {
      if (!r || r.ok !== true) {
        setStatus({ kind: "err", text: r && r.error || "\u8BFB\u53D6\u6765\u6E90\u6A21\u578B\u5931\u8D25" });
        return;
      }
      setSourceModels(r.models);
    }).catch((err) => setStatus({ kind: "err", text: String(err && err.message || err) })).finally(() => setBusyModel(false));
  };
  const onPresetModel = (model) => {
    ...
    setBusyModel(true);
    call("preset-model-info", { provider: sourceProvider, model }).then((r) => {
      if (!r || r.ok !== true) {
  ...
  const apply2 = () => {
    ...
    setBusy(true);
    setStatus(null);
    call("apply-model-config", { route: targetRoute, entry, overwrite: overwrite === true }).then((r) => {
      if (!r || r.ok !== true) {
    ...
    }).catch((err) => setStatus({ kind: "err", text: String(err && err.message || err) })).finally(() => setBusy(false));
  };
  ...
  return /* @__PURE__ */ React.createElement("div", { className: "mcfg-page" }, /* @__PURE__ */ React.createElement("h2", { className: "mcfg-title" }, t("title")), /* @__PURE__ */ React.createElement("p", { className: "mcfg-intro" }, t("intro")), boot.error ? /* @__PURE__ */ React.createElement("p", { className: "mcfg-statusErr" }, boot.error) : null, boot.writable === false ? /* @__PURE__ */ React.createElement("p", { className: "mcfg-hint" }, t("readOnly")) : null, /* @__PURE__ */ React.createElement("section", { className: "mcfg-card" }, /* @__PURE__ */ React.createElement("h3", { className: "mcfg-cardTitle" }, t("targetTitle")), /* @__PURE__ */ React.createElement("div", { className: "mcfg-field" }, /* @__PURE__ */ React.createElement("span", { className: "mcfg-label" }, t("targetRoute")), /* @__PURE__ */ React.createElement("select", { className: "mcfg-input mcfg-selectInput", value: targetRoute, onChange: (e) => {
    setTargetRoute(e.target.value);
    setLoadedEntryId("");
  } }, /* @__PURE__ */ React.createElement("option", { value: "" }, t("targetRoutePlaceholder")), boot.targets.map((x) => /* @__PURE__ */ React.createElement("option", { key: x.provider, value: x.provider }, x.displayName + (x.declared ? " \xB7 " + t("customTag") : "")))), boot.targets.length === 0 ? /* @__PURE__ */ React.createElement("p", { className: "mcfg-hint" }, t("emptyTargets")) : null), target ? /* @__PURE__ */ React.createElement("div", { className: "mcfg-field" }, /* @__PURE__ */ React.createElement("span", { className: "mcfg-label" }, t("targetModels")), /* @__PURE__ */ React.createElement(
    "input",
    {
  ...
  ), /* @__PURE__ */ React.createElement("button", { type: "button", className: "mcfg-btn mcfg-shrink", "aria-label": t("removeLevel"), onClick: () => removeLevel(i) }, "\xD7"))), /* @__PURE__ */ React.createElement("div", { className: "mcfg-row" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "mcfg-btn", onClick: addLevel }, "+ ", t("addLevel"))), /* @__PURE__ */ React.createElement("p", { className: "mcfg-hint" }, presetInfo && !(presetInfo.reasoning && presetInfo.reasoning.efforts.length) ? t("unknownReasoning") : t("reasoningHint"))) : null), exists ? /* @__PURE__ */ React.createElement("div", { className: "mcfg-field" }, /* @__PURE__ */ React.createElement("p", { className: "mcfg-note" }, t("overwriteHint")), /* @__PURE__ */ React.createElement("label", { className: "mcfg-check" }, /* @__PURE__ */ React.createElement("input", { type: "checkbox", checked: overwrite, onChange: (e) => setOverwrite(e.target.checked) }), t("overwriteLabel"))) : null), /* @__PURE__ */ React.createElement("div", { className: "mcfg-row" }, /* @__PURE__ */ React.createElement("button", { type: "button", className: "mcfg-btnPrimary", disabled: busy || boot.writable === false || !target, onClick: apply2 }, busy ? t("applying") : t("apply")), /* @__PURE__ */ React.createElement("button", { type: "button", className: "mcfg-btn", onClick: () => setShowPreview(!showPreview) }, t("preview"))), showPreview && previewEntry ? /* @__PURE__ */ React.createElement("pre", { className: "mcfg-code" }, JSON.stringify(previewEntry, null, 2)) : null, status ? /* @__PURE__ */ React.createElement("p", { className: status.kind === "ok" ? "mcfg-statusOk" : "mcfg-statusErr", role: "status", "aria-live": "polite" }, status.text) : null);
}
